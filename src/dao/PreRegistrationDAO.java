...
public class PreRegistrationDAO {
    ...
    // Optional: check if student has taken prerequisites
    public boolean checkPrerequisite(int studentId, String prerequisiteCourseCode) {
        String sql = "SELECT * FROM preregistrations " +
                     "WHERE student_id = ? AND course_code = ? AND status = 'Completed'";

        try {
            Connection conn = DBConnection.getConnection();
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, studentId);
            ps.setString(2, prerequisiteCourseCode);
            ResultSet rs = ps.executeQuery();

            boolean hasTaken = rs.next();
            rs.close();
            ps.close();
            return hasTaken;
